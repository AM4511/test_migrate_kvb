...
#
# Note: o_sclk is an inverted generated clock. This
#       is why we use the option -invert in the
#       create_generated_clock command


# Define the source clock point for the MRAM controller
set MRAM_SRC_SCK_PERIOD 20.00
set SRC_PIN_CLK50MHz   {reconfig_pll|altpll_component|auto_generated|pll1|clk[0]}


# Define the QSPI read clock (Generated clock in the file llqspi.v)
create_generated_clock -name {mram_sck_read}  -source $SRC_PIN_CLK50MHz -divide_by 4 -invert  -phase 180 -add  [get_pins { u0|qspi_mram_0|qspi_top_inst|llqspi_inst|o_sck|q }]

# Define the write clock (Generated clock in the file llqspi.v)
create_generated_clock -name {mram_sck_write} -source $SRC_PIN_CLK50MHz -divide_by 2 -invert  -phase 180 -add [get_pins { u0|qspi_mram_0|qspi_top_inst|llqspi_inst|o_sck|q }]

# Both clock are asynchronous
set_clock_groups -asynchronous -group [get_clocks {mram_sck_read}] -group [get_clocks {mram_sck_write}]

# SPI Read clock period:
set MRAM_READ_SCK_PERIOD [expr $MRAM_SRC_SCK_PERIOD * 4]


# SPI Write clock period:
...
set pcb_delay_mram_sck 0.165

set output_delay_max [expr $pcb_delay_mram_cs_n + $mr10q010_tsu_min - $pcb_delay_mram_sck - $txb0108_tsk_max]
set output_delay_min [expr $pcb_delay_mram_cs_n - $mr10q010_thold_min - $pcb_delay_mram_sck + $txb0108_tsk_max]

set_output_delay -clock  mram_sck_write -min [expr $output_delay_min + $MRAM_WRITE_SCK_PERIOD/2] -reference_pin mram_sck [get_ports {mram_cs_n}]
set_output_delay -clock  mram_sck_write -max $output_delay_max -reference_pin mram_sck [get_ports {mram_cs_n}]

set_output_delay -clock  mram_sck_read -add_delay -min [expr $output_delay_min + $MRAM_READ_SCK_PERIOD/2] -reference_pin mram_sck [get_ports {mram_cs_n}]
...
#**************************************************************
set pcb_delay_mram_io_max 0.189
set pcb_delay_mram_io_min 0.149
set pcb_delay_mram_sck 0.165
...
set output_delay_max [expr $pcb_delay_mram_io_max + $mr10q010_tsu_min - $pcb_delay_mram_sck - $txb0108_tsk_max]
set output_delay_min [expr $pcb_delay_mram_io_min - $mr10q010_thold_min - $pcb_delay_mram_sck + $txb0108_tsk_max]
